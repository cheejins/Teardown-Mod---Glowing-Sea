...
function AutoButton(set, name, fontsize, paddingwidth, paddingheight, draw, spreadpad)
	...
	UiPush()
		...
		local c = Vec((not set and 1), (set and 1), 0)
		...
		if draw then
			...
			if pressed then
				set = not set
				if set then beepOn() else beepOff() end
			end
		end
